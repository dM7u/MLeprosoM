...
export async function readTeamFixtures(db, {provider, externalTeamId, ttlMs, competitionId, seasonId, now = Date.now()}) {
  ...
  try {
    ...
    let query=db.from('fixtures')
      .select('id,external_id,source_round,home_team_id,away_team_id,kickoff_at,source_status,source_stage,home_score,away_score,fetched_at,seasons!inner(external_id,competitions!inner(name,external_id))'+scoreFields)
    ...
    const {data:teams,error:namesError}=await db.from('teams').select('id,name,external_id').eq('provider',provider).in('id',ids);
    const names=new Map((namesError?[]:teams??[]).map(x=>[x.id,x.name]));
    const externalIds=new Map((namesError?[]:teams??[]).map(x=>[x.id,x.external_id]));
    const data=visible.map(x=>({...x,provider,home_external_id:externalIds.get(x.home_team_id)??null,away_external_id:externalIds.get(x.away_team_id)??null,season_external_id:x.seasons?.external_id??null,competition_external_id:x.seasons?.competitions?.external_id??null,competition:x.seasons?.competitions?.name??null,home_team:names.get(x.home_team_id)??null,away_team:names.get(x.away_team_id)??null}));
