...
import {fixtureDate,fixtureScore,fixtureStatus} from './fixture-format.mjs';
...
function FixtureList({title,fixtures}:{title:string;fixtures:StoredFixture[]}) {
  return <section className="rounded-2xl border border-neutral-800 bg-neutral-900/60 p-5 sm:p-6">
    ...
    {!fixtures.length?<p className="text-neutral-400">Sin datos</p>:<ul className="divide-y divide-neutral-800">
      {fixtures.map(f=><li key={f.id} className="py-5 first:pt-0 last:pb-0">
        <p className="text-xs text-neutral-400">{fixtureDate(f.kickoff_at)} · {fixtureStatus(f.source_status)}</p>
        <div className="mt-3 flex items-center justify-between gap-4">
          ...
          <p className="shrink-0 text-lg font-bold tabular-nums">{fixtureScore(f.home_score,f.away_score)}</p>
        </div>
      </li>)}
    </ul>}
  </section>;
...
export default async function Page() {
  ...
  const finished=snapshot.data.filter(f=>f.source_status==='finished').sort((a,b)=>(Date.parse(b.kickoff_at??'')||0)-(Date.parse(a.kickoff_at??'')||0));
  const scheduled=snapshot.data.filter(f=>f.source_status==='notstarted').sort((a,b)=>(Date.parse(a.kickoff_at??'')||Infinity)-(Date.parse(b.kickoff_at??'')||Infinity));
  const other=snapshot.data.filter(f=>!['finished','notstarted'].includes(f.source_status));
  return (
    <main className="mx-auto min-h-dvh max-w-6xl px-5 py-10 sm:px-8 sm:py-16">
      ...
      <footer className="mt-8 text-xs text-neutral-500">Fuente de los registros: BSD. Torneos y tablas en validación.</footer>
    </main>
  );
}
