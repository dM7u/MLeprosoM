...
try {
  ...
  const get = async path => { requests++; return bsdGet(path, key); };
  ...
  if (mode === '--dry-run') {
    ...
    const run = await checked(db.from('sync_runs').insert({provider:'bsd',operation:'team-fixtures',status:'running',request_count:requests}).select('id').single());
    runId=run.id;
    ...
    await checked(db.from('sync_runs').update({status:'succeeded',finished_at:new Date().toISOString()}).eq('id',runId));
  ...
  if(db&&runId) await db.from('sync_runs').update({status:'failed',finished_at:new Date().toISOString(),error_code:code}).eq('id',runId);
